function [w] = weights(A,esuel,esuel_coord,c)
[m,n,p] = size(A);
...
for k=1:p
  ...
  Q(:,3,:)=0;  
   ...
   % calculo dos pesos segum o livro Computational fluid Dynamic author Blazek
    for j=1:m
        alfa1=(esuel_coord(j,1,k)-c(k,1))/R(1,1,k)^2; % x
        alfa2=(esuel_coord(j,2,k)-c(k,2)-(R(1,2,k)/R(1,1,k))*(esuel_coord(j,1,k)-c(k,1)))/R(2,2,k)^2; % y
