function [ S_old, state ] = streamlineMPFAD_1(influx, bflux, S_old, numsline, ...
    substeps, maxsteps, tf, nw, no)
...
global wells q elem elemfluxedge faces inedge bedge courant prd streamlines cfg
...
%         'dt'       , tf);
   ...
   if numel(wells)>1
       ...
       S_old(qelem(qflux>0)) =  qsat(qflux>0);  % saturacao do injetor (inicialmente é 1)
       injcell = qelem(qflux>0);
   else
%        % this approach doesn't works!!!
%        qelem = [bedge(bflux<0,3);bedge(bflux>0,3)]; qsat = [];
%        qflux = -[bflux(bflux<0); bflux(bflux>0)];   prd  = bedge(bflux>0,3);
%        injcell = qelem(qflux>0);
   ...
   while any(vst==0)
      ...
      if any(cfg.streamline.null_region > 0)
      ...
      cfl  = [cfl; mat2cell(vertcat(cn{:})./(n+1),ones(numel(s),1))];
   ...
   S_old = sat(:,1);
   % modified by JCT
   if ~isempty(qsat), S_old(injcell) =  qsat(qflux>0); end % saturacao do injetor
...
function pos = launching(opt, qelem, qflux, qsat)
   ...
   if isempty(qsat)       
       ...
       nsline = round(opt.numsline*qflux(inj)./sum(qflux(inj)));
       ...
       location = elemfluxedge(cell_src,:) - repmat(qflux(inj),1,4);        % where is faces (w,e,s,n)?
       location = location == 0;
       dir    = dir(location');
